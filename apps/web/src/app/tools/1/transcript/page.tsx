        ...
        {view === "confirm" && (
          <div style={card}>
            ...
            <div style={{ border: "1px solid #e5e7eb", borderRadius: 8, overflow: "hidden", marginBottom: 24 }}>
              ...
              {/* Feature links */}
              {[
                { label: "Schedule builder & conflict detection", href: "/tools/1/scheduler" },
                { label: "Bidding guidance & clearing price history", href: "/tools/1/bidding" },
              ].map((item) => (
                <button key={item.href} style={{ width: "100%", padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, borderBottom: "1px solid #f3f4f6", background: "none", border: "none", cursor: "pointer", textAlign: "left" }} onClick={() => router.push(item.href)}>
                  <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#011F5B", flexShrink: 0 }} />
                  <span style={{ fontSize: 13, color: "#011F5B", fontWeight: 600 }}>{item.label}</span>
                  <span style={{ marginLeft: "auto", fontSize: 12, color: "#011F5B", fontWeight: 700 }}>View →</span>
                </button>
              ))}
            </div>
