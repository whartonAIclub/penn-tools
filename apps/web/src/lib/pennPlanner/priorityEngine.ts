...
import type { PennPlannerTask, PrioritizedTask } from "./types";
...
function buildReason(task: PennPlannerTask, daysUntil: number, typeWeight: number): string {
  const parts: string[] = [];
...
export function prioritizeTasks(tasks: PennPlannerTask[]): PrioritizedTask[] {
  const scored = tasks
    .map((task) => {
      ...
      const baseEstimate = task.estimatedMinutes ?? defaultEstimate(task.type);
      ...
      return {
        ...
        priorityReason: buildReason(task, daysUntil, typeWeight),
      };
    })
    .sort((a, b) => {
...
export function detectDeadlineStack(tasks: PennPlannerTask[]): {
  isStacked: boolean;
  stackedTasks: PennPlannerTask[];
  message: string;
} {
...
export async function generateDailySummary(
  ...
  if (pending.length === 0) return "You're all caught up! No pending tasks.";
...
Keep it warm but direct. No bullet points.`;
  ...
  try {
    ...
    return response.content || "Focus on your top priority first.";
  } catch {
